...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self.local_buffer = defaultdict(lambda: {
            "count": 0,
            "total_ttft": 0.0,
            "total_latency": 0.0,
    ...
    def _init_firebase(self):
        if not FIREBASE_AVAILABLE:
            logger.warning("firebase-admin not installed. Metrics will not be synced to Firestore.")
            return

        cred_json = os.environ.get("FIREBASE_SERVICE_ACCOUNT_JSON")
        if not cred_json:
            logger.warning("FIREBASE_SERVICE_ACCOUNT_JSON not set. Metrics will not be synced.")
            return
    ...
    def record(self, 
        ...
        if should_sync:
            ...
            import threading
            threading.Thread(target=self._sync_to_firestore, args=(model, hardware, conc_bucket, stats_to_sync)).start()
        ...
        try:
            ...
            def update_in_transaction(transaction, doc_ref, stats):
                ...
                if snapshot.exists:
                    ...
                    new_avg_ttft = (existing.get("avg_ttft", 0) * existing.get("count", 0) + stats["total_ttft"]) / new_count
                    new_avg_latency = (existing.get("avg_latency", 0) * existing.get("count", 0) + stats["total_latency"]) / new_count
                    new_avg_throughput = (existing.get("avg_throughput", 0) * existing.get("count", 0) + stats["total_throughput"]) / new_count
