...
async def test_llm_proxy_tracking():
    ...
    with (
        ...
        response = await llm_proxy(
            endpoint="http://test", api_key="key", request=request
        )
...
async def test_llm_proxy_opt_out():
    ...
    with (
        ...
        request = LLMRequest(model="gpt-4", opt_out=True)

        await llm_proxy(endpoint="http://test", api_key="key", request=request)
